...
1. **复现**：抽最小化 demo，排除无关干扰
2. **归类**：判断属于哪个机制（层叠上下文 / BFC / 外边距合并 / 包含块 / 基线对齐 / 尺寸计算）
3. **查计算样式**：开发者工具 → Elements → Computed，看最终生效值和来源
4. **找根因**：对照 `references/css-mechanisms.md` 确认机制
5. **给方案**：修复当前 bug，同时给出更稳健的写法
